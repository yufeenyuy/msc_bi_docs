...
These are vast amounts of data, generated by heterogeneous sources, that can not be processed using traditional methods efficiently.

1. Variety: Different types of data generated by system. Example audios, videos, images, text etc.
2. Volume: Data being generated in massive amounts(e.g terabytes or petabytes) by systems e.g X, youtube etc.
3. Velocity: Data being generated and transmitted very fast or near real-time.
4. Varacity: Noise and inconsistencies in data that need to be removed and fixed.
...
+ Simultaneous batch processing: Different tasks being processed by different resources are handled altogether at the same time. Usefull when processing tasks that have no dependencies. E.g separate nodes processing different datasets at the same time.
+ Sequencial batch processing: A batch is processed only when the previous batch has been processed. This is necessary when tasks have dependencies. E.g tax data must be processed before pay sleep data.
+ concurrent batch processing: The same resource can handle multiple tasks with overlapping times. Applicably by systems with limited resources but several tasks to handle.
...
The questions should be clear, coincise, relevant and explicit with regards to the business objectives. Think of the 4 Ws i.e *Who, What, Why and When*. Some of the questions could be:
...
In this phase, data sources and destination must be described in a technical specification document like a *data definition report*. The knowledge in this document is later transferred to a data dictionary which are documents containing descriptions of the information provided by the recipient. Additionally, the descriptions in the data dictionary include schema information such as data types and validation rules, and possibly entitiy relationship diagrams. As Anayst or project manager collect all information, from the client, necessary for the development of the data model, it is important that all ambiguities should be handled. The core activities in this step are:
...
In this step, entities and their relationships are determined and in an iterative process, *the conceptual up to an including the physical model* is developed. This is where the actual modelling begins.
...
3. Validate the data model with the business department, data owner, and other stakeholders. This ensures that the conceptual model alligns with the business objectives and also receive endorsement from all the parties involved.
...
There are 4 approaches to modeling data. Each approach is suitable for a particular use case and leverage both advantages and disadvantages. These approaches are: *function-driven approach, data-driven approach, object oriented approach and prototyping approach*. 

+ *Function driven approach*

Data model is designed based on the expected functionalities of the system. It is often used in software engineering and system design where emphasis is on the specific actions of the systems. E.g A system at the hospital that automatically orders critical medications when a certain threshhold is reached.

+ *Data-driven approach*

The data model is designed based on the entities and their relationships. As such it focuses is on the movement and transformation of data. This approach is widely used in analytics and machine learning scenarios. A machine learning model that clusters customers by their consumption behaviour. A model that supports the collection for this cluster analysis must be able to collect user master data and the activities of the user on the platform.

+ *Object-oriented approach*

The model is designed based on the collection of objects that makeup a system. Usually, each object has its particularities and is reusable. This approach is commonly used in software development and system design e.g development of a holiday booking system where user(objects) are allowed to make reservations like hotel rooms, taxis or uber, tours etc.

+ *Prototyping approach*

In this approach, the model is iteratively built by improving the previous one. Based on the requirements, a basic data model is built and enriched in subsequent iterations. The model supports rapid development. This approach is used in product design, system design and machine learning.
...
1. *Complete the project deliverables*: Every associated requirment defined in the project objectives related to the data model must be delivered. In the context of data analytics, other requirements could be Dashboards or Reports, development of ETL/ELT process etc.
...
+ Inheritance: The ability of a child objects to import extra properties from its parent class. Child objects may have properties only unique to them.
+ integrity: Ensures data consistency and correctnes.
